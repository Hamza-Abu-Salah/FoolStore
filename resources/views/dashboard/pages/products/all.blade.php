...
@section('content')
    <div class="page-wrapper">
        ...
        <div class="page-header">
            <div class="row ">
                <div class="col-sm-12">
                    <h3 class="page-title" style="padding-right: 32px;"><a href="">جميع المنتجات</a></h3>
                </div>
            </div>
        </div>
                        ...
                        <div class="table-inline2 d-flex justify-content-between ">
                            ...
                            <div class="d-flex">
                                <div class="d-flex dix-h2" style="margin:16px 22px 0px 10px">
                                    <h5 style="font-size:18px;color: #3CD6D5;">جميع المنتجات</h5>
                                ...
                                <table id="example" class="table-inline1 table table-striped table-bordered"
                                    ...
                                    @forelse($items as $item)
                                        <tr style="text-align: center;" id="row-{{$item->id}}">
                                            ...
                                            <td>{{ $item->category->name_ar?? 'غير متوفر' }}</td>
                                            <td>{{ $item->store->name_ar?? 'غير متوفر' }}</td>
                                            <td>{{ $item->name_ar?? 'غير متوفر' }}</td>
                                            <td>{{ $item->name_en?? 'غير متوفر' }}</td>
                                            <td>{{ $item->status? 'فعال' : 'غير فعال' }}</td>
                                            ...
                                            <th>
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu" style="direction: ltr;">
                                                        <a class="dropdown-item" href="/products/delete/{{$item->id}}"
                                                           style=" text-align: right;"> <i class="fa-solid fa-trash-can"
                                                                                           style="margin-right: 77px;"></i>
                                                            حذف </a>
                                                        <a class="dropdown-item" href="/products/toggle_accept/{{$item->id}}"
                                                           style=" text-align: right;"> <i class="fa-solid fa-check "
                                                                                           style="margin-right: 77px;"></i>
                                                            قبول </a>
                                                    </div>
                                                </div>
...
@section('scripts')
    ...
    <script>
        ...
        function groupDelete() {
            if (ids.length > 0) {
                $.post( "/products/groupDelete", {ids: ids}).done(function() {
                    ids.forEach(item => {
                        $("#row-"+item).remove()
                    })
                    $("#0").prop("checked", false)
