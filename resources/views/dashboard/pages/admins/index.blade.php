...
@section('content')
    <div class="page-wrapper">
        ...
        <div class="page-header">
            <div class="row ">
                <div class="col-sm-12">
                    <h3 class="page-title" style="padding-right: 32px;"><a href="">المشرفين</a></h3>
                </div>
            </div>
        </div>
                        ...
                        <div class="table-inline2 d-flex justify-content-between ">
                            <div style="margin:8px 70px 0px 15px">
                                ...
                                <div class="table-buttons7 d-flex fix-buttonss2  dived"
                                    ...
                                    <div class="dropdown">
                                        <button type="button" class="btn-whitees btn text-dark" id="dropdownMenuButton" data-toggle="dropdown"> إظهار /إخفاء الاعمدة
                                        </button>
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            <a class="dropdown-item" style="cursor: pointer" onclick="checkRow(1)">#</a>
                                            <a class="dropdown-item" style="cursor: pointer" onclick="checkRow(2)">الاسم</a>
                                            <a class="dropdown-item" style="cursor: pointer" onclick="checkRow(3)">الصورة</a>
                                            <a class="dropdown-item" style="cursor: pointer" onclick="checkRow(4)">البريد الالكتروني</a>
                                            <a class="dropdown-item" style="cursor: pointer" onclick="checkRow(5)">الحالة</a>
                                            <a class="dropdown-item" style="cursor: pointer" onclick="checkRow(6)">خيارات</a>
                                        </div>
                                    </div>
                            ...
                            <div class="d-flex">
                                ...
                                <div class="d-flex dix-h2" style="margin:16px 22px 0px 10px">
                                    <h5 style="font-size:18px;color: #3CD6D5;">المشرفين</h5>
                        ...
                        <div class="card-body">
                            ...
                            <div class="table-responsive" style="width: 100%;">
                                <table id="example" class="table-inline1 table table-striped table-bordered"
                                    ...
                                    <thead>
                                    <tr>
                                        <th scope="col" class="row-1" data-orderable="false"><input onchange="checkbox(0)" type="checkbox" id="0"></th>
                                        <th scope="col" class="row-2">الإسم</th>
                                        <th scope="col" class="row-3">الصورة</th>
                                        <th scope="col" class="row-4">البريد الالكتروني</th>
                                        <th scope="col" class="row-5">الحالة</th>
                                        <th scope="col" class="row-6">خيارات</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    @forelse($items as $item)
                                        <tr style="text-align: center;" id="row-{{$item->id}}">
                                            ...
                                            <td class="row-2">{{ $item->name?? 'غير متوفر' }}</td>
                                            <td class="row-3">
                                                <img src="/{{ $item->avatar?? "assets/img/avatar-03.jpg" }}"
                                                     width="30" height="30" alt="">
                                            </td>
                                            <td class="row-4">{{ $item->email?? 'غير متوفر' }}</td>
                                            <td class="row-5">
                                                @if($item->status)
                                                ...
                                                @else
                                                    <button type="button" class="btn text-white"
                                                            style="font-size: 12px;background: #3B3333">موقوف <i
                                            ...
                                            <th class="row-6">
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu" style="direction: ltr;">
                                                        <a class="dropdown-item" href="/admins/delete/{{$item->id}}" style=" text-align: right;">
                                                            <i class="fa-solid fa-trash-can"
                                                               style="margin-right: 54px;"></i>
                                                            حذف
                                                        </a>
                                                        <a class="dropdown-item" href="/admins/edit/{{$item->id}}"
...
@section('scripts')
    ...
    <script>
        ...
        function groupDelete() {
            if (ids.length > 0) {
                $.post( "/admins/groupDelete", {ids: ids}).done(function() {
                    ids.forEach(item => {
                        $("#row-"+item).remove()
                    })
                    $("#0").prop("checked", false)
